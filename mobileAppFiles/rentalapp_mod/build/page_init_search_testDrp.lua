...
--(2) sfondo del campo notti
	   ...
	   local zonetxt = display.newText("", 0,0, native.systemFontBold, 27);
...
-- SFONDO TENDINA ZONE 
       ...
       bgzonelist = display.newImageRect( imgDir.. "img_bgzonelist.png", 411, 655 ); 
...
local function get_drpItem( txt, w, x )
	local nomeZonaItem = display.newText( "", 0, 0, w, 0, native.systemFont, 23 )
	nomeZonaItem.alpha = 1; nomeZonaItem.oldAlpha = 1 
	nomeZonaItem.text = txt
	nomeZonaItem:setTextColor(51, 51, 102)
	nomeZonaItem:setReferencePoint(display.TopLeftReferencePoint);
	nomeZonaItem.size = 25
	nomeZonaItem.x = x; 
	return nomeZonaItem;
end
local DropDownGroup;
local createDropDown = function()
	...
	local mainPadX = 30;
	local mainPadY = 30;
	local mainH = display.contentHeight/2;
	local padLeft = 100;
	...
	--menuGroup:insert(overlayImg) 
	--menuGroup:insert(DropDownGroup) 
	
	local imgBg = display.newImageRect( imgDir.. "img_drp_bg.png", width, mainH ); 
	imgBg:setReferencePoint(display.TopLeftReferencePoint);
	imgBg.x = 0; imgBg.y = display.contentHeight-mainH; imgBg.alpha = 1; imgBg.oldAlpha = 1 
	--menuGroup:insert(imgBg) 
	local imgOptionsBg = display.newImageRect( imgDir.. "img_bg_white.png", width-mainPadX*2, mainH-mainPadY*2 ); 
	imgOptionsBg:setReferencePoint(display.TopLeftReferencePoint);
	imgOptionsBg.x = mainPadX; imgOptionsBg.y = display.contentHeight-mainH+mainPadY; imgOptionsBg.alpha = 1; imgOptionsBg.oldAlpha = 1 
	--menuGroup:insert(imgOptionsBg) 
	local drpContent = widget.newTableView({
		top = display.contentHeight-mainH+mainPadY,
		left = mainPadX,
		width = width-mainPadX*2,
		height = mainH-mainPadY*2,
		maskFile = "img_drp_mask.png"
	});
	-- local drpContent = widget.newScrollView{
		-- top = display.contentHeight-mainH+mainPadY,
		-- left = mainPadX,
		-- width = width-mainPadX*2,
		-- height = mainH-mainPadY*2,
		-- scrollWidth = width-mainPadX*2,
		-- scrollHeight = mainH-mainPadY*2,
		-- maskFile="img_drp_mask.png",
		-- bgColor={255, 255, 255, 1},
		-- hideBackground=true
	-- }
	-- drpContent.isHitTestMasked = true;
	-- drpContent.content.horizontalScrollDisabled = true;
	
	local currY=drpContent.height/2;
	drpContent.topPadding=currY;
	drpContent.bottomPadding=currY;
	local drpItems={};
	
	for i=1,#_G.zoneList do
		drpContent:insertRow{
			height=64,
			onEvent = function(event) event.target.reRender = false; end,
			onRender = function(event)
					local row = event.target
					local rowGroup = event.view
					row.reRender = false
					local nomeZonaItem = get_drpItem( _G.zoneList[i].title, drpContent.width, 20 )
				   rowGroup:insert(nomeZonaItem) 
				end
		}
		--local nomeZonaItem = get_drpItem( _G.zoneList[i].title, drpContent.width, 20 )
		--nomeZonaItem.y = currY; 
	    --drpContent:insert( nomeZonaItem )
		currY=currY+nomeZonaItem.height+10
	end
	overlayImg:addEventListener( "touch",  function(event) drpContent:takeFocus( event ) end )
	--menuGroup:insert(drpContent) 
end
local showDropDown = function(event)
...
-- TESTO ITEM TENDINA ZONE 
	   ...
	   nomeZonaItem = display.newText("", 0,0, native.systemFontBold, 25);
       nomeZonaItem.alpha = 1; nomeZonaItem.oldAlpha = 1 
...
--(2) regular layer 
	   local checkindate = display.newText("", 0,0, native.systemFontBold, 27);
...
--(2) regular layer 
	   ...
	   local checkoutdate = display.newText("", 0,0, native.systemFontBold, 27);
...
--(2) regular layer 
		calcologuest = display.newText("", 0,0, native.systemFontBold, 43);
		calcologuest:setTextColor(104, 96, 111)
		calcologuest.text = _G.clConfig.guestsNum;
		calcologuest.size = 43
...
--(2) regular layer 
	   ...
	   testoinizelenco = display.newText("", 0,0, native.systemFontBold, 29);
		testoinizelenco:setTextColor(104, 96, 111)
		testoinizelenco.text = _G.clConfig.selectedZone.title
		testoinizelenco.size = 29
...
--(2) regular layer 
	   ...
	   testocheckin = display.newText("", 0,0, native.systemFontBold, 25);
...
--(2) regular layer 
	   testocheckout = display.newText("", 0,0, native.systemFontBold, 30);
		testocheckout:setTextColor(104, 96, 111)
		_G.clConfig.dataCheckout = os.date( "*t",os.time(_G.clConfig.dataCheckin) + 60*60*24*_G.clConfig.nightsNum )
		testocheckout.text = JSCal:formatCustom(_G.clConfig.dataCheckout, "#d# #MM# #yy#", _G.clConfig.langCode, _G.clConfig.dataCheckout.day .."/".. _G.clConfig.dataCheckout.month .."/".. _G.clConfig.dataCheckout.year); 
		testocheckout.size = 30
