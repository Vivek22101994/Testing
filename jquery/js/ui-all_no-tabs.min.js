...
*/
JSCal = {};
JSCal.intToDate = function (ar, au, av, at, P) { if (!(ar instanceof Date)) { ar = parseInt(ar, 10); var aw = Math.floor(ar / 10000); ar = ar % 10000; var D = Math.floor(ar / 100); ar = ar % 100; ar = new Date(aw, D - 1, ar, au || 12, av || 0, at || 0, P || 0) } return ar };
...
JSCal.Single = function (opt) {
    this.Opt = {};
    var obj = this;
    obj.Opt = $.extend(obj.Opt, JSCal.SingleOptions, opt);
    var opts = obj.Opt;
    this.beforeShowDay = function (date) {
        if (opts.beforeShowDay != null)
            return opts.beforeShowDay(date);
        return [true, ""];
    };
    this.Cal_clear = function () {
    };
    this.Cal_onSelect = function (date) {
        opts.DateInt = JSCal.dateToInt(date);
        opts.Date = JSCal.intToDate(opts.DateInt);
        opts.DateTime = opts.Date.getTime();
    };
    if ($(opts.Cont).val() != "" && $(opts.Cont).val() != "0" && $(opts.Cont).val() != "00000000")
        opts.DateInt = parseInt($(opts.Cont).val());
    else
        opts.DateInt = 0;
    opts.Cal = $(opts.View).datepicker({ showAnim: "", changeMonth: opts.changeMonth, changeYear: opts.changeYear, showOtherMonths: opts.showOtherMonths, selectOtherMonths: opts.selectOtherMonths, dateFormat: opts.dtFormat, altField: opts.Cont, altFormat: "yymmdd", onSelect: function (selectedDate) { var instance = $(this).data("datepicker"); var date = $.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat, selectedDate, instance.settings); obj.Cal_onSelect(date); }, beforeShowDay: function (date) { return obj.beforeShowDay(date); } });
    if (opts.minDateInt != null) {
        opts.Cal.datepicker("option", "minDate", JSCal.intToDate(opts.minDateInt));
    }
    if (opts.maxDateInt != null) {
        opts.Cal.datepicker("option", "maxDate", JSCal.intToDate(opts.maxDateInt));
    }
    if (opts.yearRange != null) {
        opts.Cal.datepicker("option", "yearRange", "" + opts.yearRange);
    }
    if (opts.Trigger != null) $(opts.Trigger).bind("click", function () { opts.Cal.datepicker("show"); });
    if (opts.Cleaner != null) $(opts.Cleaner).bind("click", function () { $(opts.Cont).val(''); $(opts.View).val(''); });
    if (opts.DateInt != null) $(opts.Cont).val("" + opts.DateInt)
    if ($(opts.Cont).val() != "" && $(opts.Cont).val() != "0" && $(opts.Cont).val() != "00000000") {
        opts.Date = $.datepicker.parseDate('yymmdd', $(opts.Cont).val());
        opts.Cal.datepicker("setDate", opts.Date);
        obj.Cal_onSelect(opts.Date);
    }
    if (!opts.ViewIsTrigger) opts.Cal.datepicker("option", "showOn", 'none');
};
JSCal.RangeOptions = {
    dtFormat: "DD, d MM, yy",
    minDateInt: null,
...
JSCal.Range = function (opt) {
    this.Opt = {};
    var obj = this;
    obj.Opt = $.extend(obj.Opt, JSCal.RangeOptions, opt);
    var opts = obj.Opt;
    var one_day = 1000 * 60 * 60 * 24;
    this.startCal_show = function () {
        opts.startCal.popup(opts.startTrigger);
    };
    this.beforeShowDay = function (date) {
        if (opts.beforeShowDay != null)
            return opts.beforeShowDay(date);
    ...
    this.startCal_clear = function () {
        opts.startDate = null;
        opts.startCont.value = "";
        opts.startView.value = "";
    };
    this.startCal_onSelect = function (date) {
        //alert('ok3');
        opts.startDateInt = JSCal.dateToInt(date);
        opts.startDate = JSCal.intToDate(opts.startDateInt);
        opts.startDateTime = opts.startDate.getTime();
        var minDate = JSCal.intToDate(opts.startDateInt + opts.countMin);
        var endDate = opts.endCal.datepicker("getDate");
        opts.endCal.datepicker("option", "minDate", minDate);
        if (endDate == null || endDate < minDate) {
            opts.endCal.datepicker("setDate", minDate);
            obj.endCal_onSelect(minDate);
            obj.countVal_set();
        }
        else {
            obj.countVal_set();
    ...
    this.startCal_onClose = function (date) {

        //if (opts.endTrigger != null) $(opts.endTrigger).click();
        //setTimeout(1000, $("#" + opts.endTrigger.replace("#", "")).click());
        //opts.endCal.trigger('focus');
        //opts.endCal.datepicker("show")

        if (opts.endTrigger != null) {
            setTimeout(function () {
                $("#" + opts.endView.replace("#", "")).focus()
            }, 0);
        }
    };

    this.endCal_onSelect = function (date) {
        opts.endDateInt = JSCal.dateToInt(date);
        opts.endDate = JSCal.intToDate(opts.endDateInt);
        opts.endDateTime = opts.endDate.getTime();
        obj.countVal_set();
        if (opts.callBack)
            opts.callBack();
    };
    this.countVal_set = function () {
        //alert('opts.endDateTime=' + opts.endDateTime + ';opts.startDateTime=' + opts.startDateTime + ';opts.countCont=' + opts.countCont);
        if (opts.endDateTime == null || opts.endDateTime == 0 || opts.startDateTime == null || opts.startDateTime == 0 || opts.countCont == null) return;
        opts.countLast = opts.countVal;
        opts.countVal = Math.ceil(((opts.endDateTime - opts.startDateTime - 3800000) / (one_day)));
        if (opts.countCont != null) {
            //alert(opts.endDateTime + '/' + opts.startDateTime);
            console.log(opts.countLabel);
            $(opts.countCont).val(opts.countVal + " " + opts.countLabel);
        }
    };
    this.countVal_onChange = function (newCount) {
        if (newCount < opts.countMin) newCount = opts.countMin;
        if (newCount > opts.countMax) newCount = opts.countMax;
        var newDate = JSCal.intToDate(opts.startDateInt + newCount);
        if (newDate != opts.endDate) {
            opts.endCal.datepicker("setDate", newDate);
            obj.endCal_onSelect(newDate);
            if (opts.callBack)
                opts.callBack();
        }
    };
    this.countVal_validateChange = function () {
        var _count = parseInt($(opts.countCont).val(), 10);
    ...
    if ($(opts.startCont).val() != "" && $(opts.startCont).val() != "0" && $(opts.startCont).val() != "00000000")
        opts.startDateInt = parseInt($(opts.startCont).val());
    else
        opts.startDateInt = 0;
    if ($(opts.endCont).val() != "" && $(opts.endCont).val() != "0" && $(opts.endCont).val() != "00000000")
        opts.endDateInt = parseInt($(opts.endCont).val());
    else
        opts.endDateInt = 0;
    opts.startCal = $(opts.startView).datepicker({
        showAnim: "", changeMonth: opts.changeMonth, changeYear: opts.changeYear, showOtherMonths: opts.showOtherMonths, selectOtherMonths: opts.selectOtherMonths, dateFormat: opts.dtFormat, altField: opts.startCont, altFormat: "yymmdd", onSelect: function (selectedDate) {
            var instance = $(this).data("datepicker"); var date = $.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat, selectedDate, instance.settings); obj.startCal_onSelect(date);
            obj.startCal_onClose(date);
        }, beforeShowDay: function (date) { return obj.beforeShowDay(date); }
    });
    opts.endCal = $(opts.endView).datepicker({ showAnim: "", changeMonth: opts.changeMonth, changeYear: opts.changeYear, showOtherMonths: opts.showOtherMonths, selectOtherMonths: opts.selectOtherMonths, dateFormat: opts.dtFormat, altField: opts.endCont, altFormat: "yymmdd", onSelect: function (selectedDate) { var instance = $(this).data("datepicker"); var date = $.datepicker.parseDate(instance.settings.dateFormat || $.datepicker._defaults.dateFormat, selectedDate, instance.settings); obj.endCal_onSelect(date); }, beforeShowDay: function (date) { return obj.beforeShowDay(date); } });
    if (opts.minDateInt != null) {
        opts.startCal.datepicker("option", "minDate", JSCal.intToDate(opts.minDateInt));
        opts.endCal.datepicker("option", "minDate", JSCal.intToDate(opts.minDateInt));
    }
    if (opts.maxDateInt != null) {
        opts.startCal.datepicker("option", "maxDate", JSCal.intToDate(opts.maxDateInt));
        opts.endCal.datepicker("option", "maxDate", JSCal.intToDate(opts.maxDateInt));
    }
    if (opts.startTrigger != null) $(opts.startTrigger).bind("click", function () { opts.startCal.datepicker("show"); });
    if (opts.endTrigger != null) $(opts.endTrigger).bind("click", function () { opts.endCal.datepicker("show"); });
    $(opts.startCont).val("" + opts.startDateInt)
    $(opts.endCont).val("" + opts.endDateInt)
    if ($(opts.startCont).val() != "" && $(opts.startCont).val() != "0" && $(opts.startCont).val() != "00000000") {
        //alert('ok1');
        opts.startDateInt = parseInt($(opts.startCont).val());
        opts.startDate = $.datepicker.parseDate('yymmdd', $(opts.startCont).val());
        opts.startCal.datepicker("setDate", opts.startDate);
    }
    if ($(opts.endCont).val() != "" && $(opts.endCont).val() != "0" && $(opts.endCont).val() != "00000000") {
        //alert('ok2');
        opts.endDateInt = parseInt($(opts.endDate).val());
        opts.endDate = $.datepicker.parseDate('yymmdd', $(opts.endCont).val());
        opts.endCal.datepicker("setDate", opts.endDate);
    }
    obj.startCal_onSelect(opts.startDate);
    obj.endCal_onSelect(opts.endDate);
    if (!opts.ViewIsTrigger) { opts.startCal.datepicker("option", "showOn", 'none'); opts.endCal.datepicker("option", "showOn", 'none'); }
    obj.countVal_set();
};
...
(function (d, A) {
    ...
    } d.extend(d.ui, { datepicker: { version: "1.8.12" } }); var y = (new Date).getTime(); d.extend(K.prototype, {
                ...
                for (var M = "", E = 0; E < i[1]; E++) {
                    var N = this._daylightSavingAdjust(new Date(m, g, a.selectedDay)), t = " ui-corner-all", x = ""; if (l) {
                        ...
                        x += "<tr>"; var P = !j ? "" : '<td class="ui-datepicker-week-col">' + this._get(a, "calculateWeek")(q) + "</td>"; for (t = 0; t < 7; t++) {
                            var G =
                            p ? p.apply(a.input ? a.input[0] : null, [q]) : [true, ""], C = q.getMonth() != g, J = C && !H || !G[0] || k && q < k || o && q > o; P += '<td id="' + JSCal.dateToInt(q) + '" class="' + ((t + h + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (C ? " ui-datepicker-other-month" : "") + (q.getTime() == N.getTime() && g == a.selectedMonth && a._keyEvent || L.getTime() == q.getTime() && L.getTime() == N.getTime() ? " " + this._dayOverClass : "") + (J ? " " + this._unselectableClass + " ui-state-disabled" : "") + (C && !v ? "" : " " + G[1] + (q.getTime() == u.getTime() ? " " + this._currentClass : "") + (q.getTime() == b.getTime() ? " ui-datepicker-today" :
                            "")) + '"' + ((!C || v) && G[2] ? ' title="' + G[2] + '"' : "") + (J ? "" : ' onclick="DP_jQuery_' + y + ".datepicker._selectDay('#" + a.id + "'," + q.getMonth() + "," + q.getFullYear() + ', this);return false;"') + ">" + (G[3] ? '' + G[3] + '' : "") + (C && !v ? "&#xa0;" : J ? '<span class="ui-state-default">' + q.getDate() + "</span>" : '<a class="ui-state-default' + (q.getTime() == b.getTime() ? " ui-state-highlight" : "") + (q.getTime() == u.getTime() ? " ui-state-active" : "") + (C ? " ui-priority-secondary" : "") + '" href="#">' + q.getDate() + "</a>") + "</td>"; q.setDate(q.getDate() + 1); q = this._daylightSavingAdjust(q)
                        } x +=
                        P + "</tr>"
