...
{
    public partial class EstateChnlHoliday_media : adminBasePage
    {
        ...
        protected void pnlDett_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            ...
            LV_gallery_DataBind();
            ChnlHolidayUpdate.UpdatePhotos_start(IdEstate, App.HOST);
        }
        protected void LV_gallery_DataBind()
        {
            ...
            if (LV_gallery.Items.Count < ChnlHolidayUtils.PropertyHasPhotos(IdEstate))
            {
                var str = "";
                str += "Sono presenti delle foto caricate direttamente online:";
                str += "<br/>";
                str += "1) Se caricate una foto, si aggiungerà a quelle già presenti che non sono visualizzate in questa sezione e non è permesso cancellare da remoto.";
                str += "<br/>";
                str += "2) Potete cancellare le foto online e gestire tutto il caricamento da questa sezione";
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "errorAlert", "$(document).ready(function () {$(window).load(function () {radalert(\"Attenzione!<br />" + str + "\", 500, 150);});});", true);
            }
        }
