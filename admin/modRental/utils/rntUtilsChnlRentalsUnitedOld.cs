...
public class ChnlRentalsUnitedUtils
{
    ...
    public static void StopTimers()
    {
        try
        {
            if (timerChnlRentalsUnitedImport != null)
            {
                timerChnlRentalsUnitedImport.Stop();
        ...
        {
            ErrorLog.addLog("", "ChnlRentalsUnitedUtils.StopTimers", ex.ToString() + "");
        }
    }
    ...
    private class ClearLog_process
    {
        void doThread()
        {
            ...
            {
                ErrorLog.addLog("", "ChnlRentalsUnitedUtils.ClearLog_process", Ex.ToString() + "");
            }
        }
    ...
    public static void updateEstateFromMagarental(RNT_TB_ESTATE currEstate)
    {
            ...
            using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
            {
                ...
                if (lstEstate != null && lstEstate.Count > 0)
                {
                    foreach (RNT_LN_ESTATE objEstate in lstEstate)
                    {
                        ...
                        currRentalsUnitedLN.title = objEstate.title;
                        ...
                        currRentalsUnitedLN.description = objEstate.description;
                        dcChnl.SaveChanges();
                    }
                }
        ...
        {
            ErrorLog.addLog("", "ChnlRentalsUnitedUtils.updateEstateFromMagarental RoomTB, IdEstate: " + currEstate.id, ex.ToString());
        }
    }
    ...
    public static bool CheckAvailability(RNT_TB_ESTATE currEstateTB, DateTime dtStart, DateTime dtEnd, long agentID, int supportMultiUnit)
    {
        ...
        var closedList = new List<dbRntChnlRentalsUnitedEstateClosedDatesRL>();
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
        {
            closedList = dcChnl.dbRntChnlRentalsUnitedEstateClosedDatesRLs.Where(x => x.pidEstate == currEstateTB.id && x.changeDate >= dtStart && x.changeDate < dtEnd && x.isClosed > 0).ToList();
        }
        ...
        {
            //bool isAnyReseration = resList.Any(x => x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia);
        ...
        void doThread()
        {
            try
            {
                ...
                using (var DC_RENTAL = maga_DataContext.DC_RENTAL)
                {
                    ...
                    var agentTbl = dcRnt.dbRntAgentTBLs.FirstOrDefault(x => x.isActive == 1 && x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia && x.is_integrate == 1);
                    if (agentTbl != null)
                        agentID = agentTbl.id;
                    if (agentID == 0) return;
...
public class ChnlRentalsUnitedService
{
    ...
    public static string ListingContentIndexRequest()
    {
        ...
        long agentID = 0;
        ChnlRentalsUnitedClasses.ErrorList errorList = new ChnlRentalsUnitedClasses.ErrorList("listingContentIndex");
    ...
    public static string BookingContentIndexRequest(string xml)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
            ...
            {
                errorList.AddError("OTHER");
                ErrorLog.addLog("", "BookingContentIndexRequest", ex.ToString());
            }
        return errorList.GetXml();
    }
    ...
    public static string Listing(int IdEstate)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
        {
            ...
            if (currEstate == null)
            {
                return ChnlRentalsUnitedClasses.ErrorList.GetErrorXml("OTHER", "listing");
            }
            ...
            var notSupportedLangIds = CommonUtilities.getSYS_SETTING("rnt_RUNotSupportedLangIds").splitStringToList(",");
            var currLnList = dcChnl.dbRntChnlRentalsUnitedEstateLNs.Where(x => x.pid_estate == IdEstate).ToList();

            #region not supported language from RentalsUnited
            ...
            {
                Response.adContent.accommodationsSummary.Add(ln.summary, ln.pid_lang);
                Response.adContent.description.Add(ln.description, ln.pid_lang);
                Response.adContent.headline.Add(ln.title, ln.pid_lang);
                Response.adContent.ownerListingStory.Add(ln.listing_story, ln.pid_lang);
                Response.adContent.propertyName.Add(CurrentSource.rntEstate_title(IdEstate, ln.pid_lang.objToInt32(), currEstate.code), ln.pid_lang);
                Response.adContent.uniqueBenefits.Add(ln.unique_benifits, ln.pid_lang);
                Response.adContent.whyPurchased.Add(ln.why_purchased, ln.pid_lang);
                Response.location.description.Add(ln.location_description, ln.pid_lang);
                Response.location.otherActivities.Add(ln.location_other_activities, ln.pid_lang);
            }

            //AD CONTENT
            Response.adContent.yearPurchased = currRUEstate.year_purchased;
            ...
            Response.location.showExactLocation = currRUEstate.show_exact_location == 1;

            var images = DC_RENTAL.RNT_RL_ESTATE_MEDIAs.Where(x => x.type == "chnlRU" && x.pid_estate == IdEstate).OrderBy(x => x.sequence).ToList();
            ...
            foreach (var unit in Units)
            {
                ...
                currUnit.maxSleepInBeds = unit.num_max_sleep_bed.objToInt32();
                currUnit.diningSeating = unit.num_dining_seating;
                ...
                foreach (var room in bathRooms)
                {
                    ...
                    var tmpRoomLns = dcChnl.dbRntChnlRentalsUnitedEstateRoomLNs.Where(x => x.pidRoom == room.uid).ToList();
                ...
                foreach (var ln in currLnList)
                {
                    currUnit.bathroomDetails.Add(ln.bathroom_details, ln.pid_lang);
                    currUnit.bedroomDetails.Add(ln.bedroom_details, ln.pid_lang);
                    currUnit.description.Add(ln.unit_description, ln.pid_lang);
                    currUnit.featuresDescription.Add(ln.features_description, ln.pid_lang);
                    currUnit.unitName.Add(ln.unit_name, ln.pid_lang);
                }
    ...
    public static string ListingRate(int IdEstate)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
        {
            ...
            if (ownerTbl == null)
            {
                ErrorLog.addLog("", "UnitRate", "Owner for RentalsUnited was not found or not active");
                return "";
            }
            RNT_TB_ESTATE currEstate = AppSettings.RNT_TB_ESTATE.SingleOrDefault(x => x.id == IdEstate && x.is_active == 1);
            if (currEstate == null)
            {
                return ChnlRentalsUnitedClasses.ErrorList.GetErrorXml("OTHER", "unitRatePeriods");
            }
            dbRntChnlRentalsUnitedEstateTB currRUEstate = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == IdEstate);
            if (currRUEstate == null)
            {
            ...
            //var Units = dcChnl.dbRntChnlRentalsUnitedEstateTBs.Where(x => x.id == IdEstate || (x.is_slave == 1 && x.pid_master_estate == IdEstate)).ToList();
            var unit = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == IdEstate);
            // var currLnList = dcChnl.dbRntChnlRentalsUnitedEstateLNs.Where(x => x.pid_estate == IdEstate).ToList();
    ...
    public static string ListingAvailability(int IdEstate)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
        {
            ...
            if (ownerTbl == null)
            {
                ErrorLog.addLog("", "UnitAvailability", "Owner for RentalsUnited was not found or not active");
                return "";
            }
            RNT_TB_ESTATE currEstate = AppSettings.RNT_TB_ESTATE.SingleOrDefault(x => x.id == IdEstate && x.is_active == 1);
            if (currEstate == null)
            {
                return ChnlRentalsUnitedClasses.ErrorList.GetErrorXml("OTHER", "unitAvailabilityEntities");
            }
            dbRntChnlRentalsUnitedEstateTB currRUEstate = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == IdEstate);
            if (currRUEstate == null)
            {
            ...
            //var Units = dcChnl.dbRntChnlRentalsUnitedEstateTBs.Where(x => x.id == IdEstate || (x.is_slave == 1 && x.pid_master_estate == IdEstate)).ToList();
            var unit = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == IdEstate);
            //var currLnList = dcChnl.dbRntChnlRentalsUnitedEstateLNs.Where(x => x.pid_estate == IdEstate && x.pid_lang != "enu").ToList();
            ...
            DateTime dtCurrent = dtStart;
            int outError;
            var priceListPerDates = rntUtils.estate_getPriceListPerDates(IdEstate,0, dtStart, dtEndTemp, out outError);
            ...
            var agentTbl = dc.dbRntAgentTBLs.FirstOrDefault(x => x.isActive == 1 && x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia);
            if (agentTbl.supportMultiUnit.objToInt32() == 0)//MultiUnit not supported
            {
                while (dtCurrent < dtEndTemp)
                {
                    var tmpDate = closedList.FirstOrDefault(x => x.changeDate == dtCurrent);
                    bool isAvv = false;
                    //bool isAnyReseration = resList.Any(x => x.dtStart.Value <= dtCurrent && x.dtEnd > dtCurrent && x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia);
                    ...
                    if (isAvv)
                    {
                        int baseAvv = 1;
                        var units = baseAvv - resList.Where(x => x.dtStart.Value <= dtCurrent && x.dtEnd > dtCurrent).Count();
                        isAvv = (tmpDate != null && tmpDate.isClosed == 1) ? false : units > 0;
                    ...
                    //if (priceList != null)
                    //{
                    //    if (!priceList.lstClosedArrival.Contains(dtCurrent.getIndex_DayOfWeek()) && !priceList.lstClosedDeparture.Contains(dtCurrent.getIndex_DayOfWeek()))
                    //        changeOver = "C";
                    //    else if (!priceList.lstClosedArrival.Contains(dtCurrent.getIndex_DayOfWeek()) && priceList.lstClosedDeparture.Contains(dtCurrent.getIndex_DayOfWeek()))
                    //        changeOver = "I";
                    //    else if (priceList.lstClosedArrival.Contains(dtCurrent.getIndex_DayOfWeek()) && !priceList.lstClosedDeparture.Contains(dtCurrent.getIndex_DayOfWeek()))
                    //        changeOver = "O";
                    //}
                    ...
                    Response.unitAvailability.unitAvailabilityConfiguration.minStay.Add((tmpDate != null && tmpDate.minStay > 0) ? tmpDate.minStay + "" : (priceList != null ? priceList.MinNights + "" : currEstate.nights_min.objToInt32() + ""));
            ...
            else
            {
                while (dtCurrent < dtEndTemp)
                {
                    ...
                    int baseAvv = 1;
                    var units = baseAvv - resList.Where(x => x.dtStart.Value <= dtCurrent && x.dtEnd > dtCurrent).Count();
                    bool isAvv = (tmpDate != null && tmpDate.isClosed == 1) ? false : units > 0;
                    //bool isAvv = resList.Where(x => x.dtStart.Value <= dtCurrent && x.dtEnd > dtCurrent).Count() == 0;
    ...
    public static string Booking(long id)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
        {
            ...
            long agentID = 0;
            var agentTbl = dc.dbRntAgentTBLs.FirstOrDefault(x => x.isActive == 1 && x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia && x.is_integrate == 1);
            if (agentTbl != null)
                agentID = agentTbl.id;
            if (agentID == 0)
            {
            ...
            dbRntChnlRentalsUnitedEstateTB currRUEstate = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == reservationTbl.pid_estate.objToInt32());
            if (currRUEstate == null)
            {
                return ChnlRentalsUnitedClasses.ErrorList.GetErrorXml("OTHER", "bookingResponse");
            }
            ...
            else
            {
                ...
                {
                    USR_TBL_CLIENT userClientTBL = maga_DataContext.DC_USER.USR_TBL_CLIENT.SingleOrDefault(x => x.id == reservationTbl.cl_id);
            ...
            {
                USR_TBL_OWNER currOwner = maga_DataContext.DC_USER.USR_TBL_OWNER.SingleOrDefault(x => x.id == currEstateTB.pid_owner);
                if (currOwner != null)
                    taxId = currOwner.invTaxId.objToInt32();
            }

            if (taxId == 0 || invProps.CashTaxLK.SingleOrDefault(x => x.id == taxId) == null)
                taxPercentage = CommonUtilities.getSYS_SETTING("rnt_DefaultRUTax").objToDecimal();
            else
            {
            ...
            dbRntEstateAgentContractRL currEstateContract = dc.dbRntEstateAgentContractRLs.FirstOrDefault(x => x.pidAgent == agentID && x.pidEstate == reservationTbl.pid_estate);
            if (currEstateContract != null)
            {
                dbRntAgentContractTBL currContract = dc.dbRntAgentContractTBLs.SingleOrDefault(x => x.id == currEstateContract.pidAgentContract);
                if (currContract != null)
                {
                    if (currContract.contractType == "PPB")
                        Response.advertiserAssignedId = ownerTbl.ppb_advertiserAssignedId;
                    else
            ...
            if (currRUEstate.is_slave == 1 && currRUEstate.pid_master_estate.objToInt32() > 0)
                Response.listingExternalId = currRUEstate.pid_master_estate.objToInt32() + "";
            else
                Response.listingExternalId = currRUEstate.id.objToInt32() + "";
            Response.unitExternalId = currRUEstate.id + "";
            Response.externalId = reservationTbl.id + "";
            Response.locale = contUtils.getLang_commonName(reservationTbl.cl_pid_lang).Replace("-", "_");
            ...
            orderItem.name = "Rent";
            orderItem.preTaxAmount.ValueDecimal = getPriceWithoutTax((reservationTbl.pr_total.objToDecimal() - totalExtras), taxPercentage);
            ...
            foreach (var extra in currResExtras)
            {
                orderItem = new ChnlRentalsUnitedClasses.OrderItem();
                order.orderItemList.Add(orderItem);
                orderItem.feeType = "MISC";
                orderItem.name = orderItem.description = rntUtils.getEstateExtras_title(extra.pidExtra, App.DefLangID, "");
                orderItem.preTaxAmount.ValueDecimal = getPriceWithoutTax((extra.price.objToDecimal()), taxPercentage);
            ...
            //Part Payment
            var paymentScheduleItem = new ChnlRentalsUnitedClasses.PaymentScheduleItem();
            if (reservationTbl.pr_part_payment_total > 0)
            {
                paymentScheduleItem.amount.ValueDecimal = reservationTbl.pr_part_payment_total.objToDecimal();
                paymentScheduleItem.dueDate.ValueDate = reservationTbl.dtCreation;
                if (paymentScheduleItem.dueDate.ValueDate < DateTime.Now.Date) paymentScheduleItem.dueDate.ValueDate = DateTime.Now.Date;
                order.paymentSchedule.paymentScheduleItemList.Add(paymentScheduleItem);
            }

            //Balance Payment
            if (reservationTbl.pr_part_owner > 0)
            {
            ...
            if (currRUEstate.CancellationPolicyCharge.objToDecimal() > 0)
            {
                ...
                cancellationPolicyItem.amount.ValueDecimal = currRUEstate.CancellationPolicyCharge.objToDecimal();
                if (currRUEstate.CancellationPolicyDeadlineDaysBeforeCheckIn.objToInt32() != 0)
                    cancellationPolicyItem.deadline.ValueDate = (reservationTbl.dtStart.Value.AddDays(-currRUEstate.CancellationPolicyDeadlineDaysBeforeCheckIn.Value));
                cancellationPolicyItem.penaltyType = currRUEstate.CancellationPolicyType;
            }

            order.reservationCancellationPolicy.description = App.HOST + contUtils.getStp_pagePath(CommonUtilities.getSYS_SETTING("ruCancalltionPolicyPage").objToInt32(), "en");
            ...
            Response.rentalAgreement.agreementText = App.HOST + contUtils.getStp_pagePath(3, "en");
            return Response.GetXml();
        }
    }
    ...
    public static string QuoteRequest(string xml)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
            try
            {
                ...
                dbRntChnlRentalsUnitedEstateTB currRUEstate = dcChnl.dbRntChnlRentalsUnitedEstateTBs.SingleOrDefault(x => x.id == Request.unitExternalId.ToInt32());
                if (currRUEstate == null)
                {
                    errorList.AddError("UNKNOWN_PROPERTY");
                    return errorList.GetXml();
                ...
                if (currEstateTB.priceVersion == 4)
                {
                    var lastDay = dc.dbRntEstatePriceV4DatesVIEWs.FirstOrDefault(x => x.dtEnd >= dtEnd && x.dtStart <= dtEnd && x.pidEstate == currEstateTB.id);
                    if (lastDay == null)
                    {
                        errorList.AddError("END_DAY_MISMATCH");
                        return errorList.GetXml();
                ...
                else if (currEstateTB.priceVersion == 5)
                {
                    var lastDay = dc.dbRntEstatePriceV5DatesVIEWs.FirstOrDefault(x => x.dtEnd >= dtEnd && x.dtStart <= dtEnd && x.pidEstate == currEstateTB.id);
                    if (lastDay == null)
                    {
                        errorList.AddError("END_DAY_MISMATCH");
                        return errorList.GetXml();
                ...
                {
                    var currFeature = dcChnl.dbRntChnlRentalsUnitedEstateFeaturesRLs.SingleOrDefault(x => x.pidEstate == currRUEstate.id && x.code == "SUITABILITY_PETS_ASK");
                    if (currFeature == null || currFeature.count < Request.reservation.numberOfPets.objToInt32())
                    {
                        var currFeature_PETS_CONSIDERED = dcChnl.dbRntChnlRentalsUnitedEstateFeaturesRLs.SingleOrDefault(x => x.pidEstate == currRUEstate.id && x.code == "SUITABILITY_PETS_CONSIDERED");
                ...
                if (currRUEstate.num_max_sleep.objToInt32() == 0)
                {
                    currRUEstate.num_max_sleep = currEstateTB.num_persons_max;
                }
                ...
                if (currRUEstate.num_max_sleep > 0 && currRUEstate.num_max_sleep < (Request.reservation.numberOfAdults.objToInt32() + Request.reservation.numberOfChildren.objToInt32()))
                {
                ...
                bool _isAvailable = ChnlRentalsUnitedUtils.CheckAvailability(currEstateTB, dtStart, dtEnd, agentTbl.id, agentTbl.supportMultiUnit.objToInt32());
                ...
                #endregion
                decimal price = rntUtils.rntEstate_getPrice_HA(0, Request.unitExternalId.objToInt32(), ref outPrice, ChnlRentalsUnitedProps.IdAdMedia);
                ...
                orderItem.description = "Rent";
                orderItem.name = "Rent";
                orderItem.preTaxAmount.ValueDecimal = getPriceWithoutTax((price - outPrice.prOptioniExtra), taxPercentage);
                orderItem.totalAmount.ValueDecimal = price - outPrice.prOptioniExtra;
                ...
                //Part Payment
                var paymentScheduleItem = new ChnlRentalsUnitedClasses.PaymentScheduleItem();
                if (outPrice.pr_part_payment_total > 0)
                {
                ...
                if (currRUEstate.CancellationPolicyCharge.objToDecimal() > 0)
                {
                    ...
                    cancellationPolicyItem.amount.ValueDecimal = currRUEstate.CancellationPolicyCharge.objToDecimal();
                    if (currRUEstate.CancellationPolicyDeadlineDaysBeforeCheckIn.objToInt32() != 0)
                        cancellationPolicyItem.deadline.ValueDate = (dtStart.AddDays(-currRUEstate.CancellationPolicyDeadlineDaysBeforeCheckIn.Value));
                    cancellationPolicyItem.penaltyType = currRUEstate.CancellationPolicyType;
                }
                order.reservationCancellationPolicy.description = App.HOST + contUtils.getStp_pagePath(CommonUtilities.getSYS_SETTING("ruCancalltionPolicyPage").objToInt32(), "en");

                //set stay fees for city tax
                if (!string.IsNullOrEmpty(contUtils.getLabel("lblRUStayFess")) && contUtils.getLabel("lblRUStayFess") != "lblRUStayFess")
                {
            ...
            {
                errorList.AddError("OTHER");
                ErrorLog.addLog("", "RU_Quote", ex.ToString());
            }
        return errorList.GetXml();
    }
    ...
    public static string BookingRequest(string xml)
    {
        ...
        using (DCchnlRentalsUnited dcChnl = new DCchnlRentalsUnited())
            try
            {
                ...
                if (ownerTbl == null)
                {
                    ErrorLog.addLog("", "BookingRequest", "Owner for RentalsUnited was not found or not active");
                    return "";
                }
                var agentTbl = dc.dbRntAgentTBLs.FirstOrDefault(x => x.isActive == 1 && x.IdAdMedia == ChnlRentalsUnitedProps.IdAdMedia && x.is_integrate == 1);
                if (agentTbl != null)
                ...
                if (agentID == 0)
                {
                    ErrorLog.addLog("", "BookingRequest", "Agent for RentalsUnited was not found or not active");
                    return "";
                }
                ...
                //outPrice.part_percentage = currEstateTB.pr_percentage.objToDecimal();
                #endregion
                decimal prTotal = rntUtils.rntEstate_getPrice_HA(0, Request.unitExternalId.objToInt32(), ref outPrice, ChnlRentalsUnitedProps.IdAdMedia);
                if (prTotal == 0)
                {
                    if (outPrice.outError == 1)
                        errorList.AddError("MIN_STAY_NOT_MET");
                ...
                if (orderItem != null)
                {
                    ...
                    newRes.requestFullPayAccepted = 0;
                    ...
                    int roomNumber = rntUtils.getEstateRoomNumber(newRes, currEstateTB.baseAvailability.objToInt32());
                    newRes.pidEstateRoomNumber = roomNumber;
            ...
            {
                errorList.AddError("OTHER");
                ErrorLog.addLog("", "RU_Booking", ex.ToString());
            }
        return errorList.GetXml();
    }
